...
- **Hospedagem**: GitHub Pages, servindo `index.html` direto da raiz da branch `main`
  (repositório público — Pages em repo privado exige GitHub Pro, ver `CHANGELOG.md`). Sem
  servidor, sem build — só `git push`.
  - **Login @turbi.com.br — pausado (2026-08-26)**: existe uma tentativa (revertida) de também
    servir a página via uma implantação restrita do próprio Apps Script, pra exigir login Google
    antes de mostrar qualquer coisa. O `HtmlService` do Apps Script se mostrou instável demais
    pra servir HTML grande como resposta de Web App (ver `CHANGELOG.md`, entradas de 2026-08-26,
    pro histórico completo da investigação) — revertido de propósito pra manter o painel estável
    enquanto essa hospedagem é redesenhada com calma numa rodada futura. **Ainda existem 2
    implantações "logadas" no projeto Apps Script** (criadas durante essa tentativa, uma delas por
    engano) — ambas atualmente rodam a MESMA API pura da implantação pública (sem servir página
    nenhuma), só como precaução pra não deixar nenhuma URL com código quebrado no ar.
...
O site é público e sem login (GitHub Pages). A nuvem de palavras (Damage/Wash/POD — comentários de clientes)
...
`Code.gs` é API pura hoje (sem servir página nenhuma — ver `CHANGELOG.md` 2026-08-26 pro histórico
da tentativa de hospedagem logada, revertida por ora). Só existe **uma implantação relevante**: a
pública, usada pelo GitHub Pages.
...
1. Editar `apps-script/Code.gs` normalmente.
2. Copiar pro nome que o projeto Apps Script realmente usa (fora do git, só existe como artefato
   de push, nunca editar direto):
   ```bash
   cp apps-script/Code.gs "apps-script/Código.gs"   # nome legado do arquivo lá, não vale renomear
   ```
3. `clasp push --force` de dentro de `apps-script/` — sobe `appsscript.json` + `Código.gs` pro
   "HEAD" do projeto.
4. `clasp deploy -i <ID_DA_IMPLANTACAO_PUBLICA> -d "descrição"` (`clasp deployments` lista o ID).
5. Validar: `curl` no `?endpoint=indisponibilidade` (ou qualquer outro) e conferir que volta JSON.
...
tocar, sobrescrevendo o que ela já tinha — **não é uma config fixa por implantação**. Se algum dia
existir mais de uma implantação de novo (ver nota abaixo), sempre conferir/ajustar
`webapp.access` (`"ANYONE_ANONYMOUS"` pra pública) antes de rodar `clasp deploy -i <id>`, e
confirmar com `curl` que a implantação pública continua respondendo JSON sem pedir login depois de
qualquer deploy — já derrubou o GitHub Pages por alguns minutos uma vez.

### Via editor do Apps Script (manual, fallback)
...
4. `Implantar → Gerenciar implantações → editar (ícone de lápis) na implantação pública → Nova
   versão → Implantar`. **Importante**: reimplantar sem criar uma "Nova versão" mantém o código
   antigo no ar.
5. Conferir em **"Quem pode acessar"** que continua **"Qualquer pessoa"** antes de sair.

### Nota — implantações "logadas" pausadas

Durante uma tentativa de hospedagem com login @turbi.com.br (2026-08-26, revertida — ver
`CHANGELOG.md`), acabaram existindo **2 implantações extras** do mesmo projeto (uma criada de
propósito, outra por engano clicando "Nova implantação"). As duas foram deixadas rodando a MESMA
API pura da pública (não servem página nenhuma), só por precaução — não precisam de manutenção até
essa hospedagem ser redesenhada. Se quiser limpar, pode deletar as duas em `Implantar → Gerenciar
implantações` sem afetar nada — só a implantação pública importa hoje.
...
- **Alguém reativou a hospedagem logada e apareceu `SyntaxError: Unexpected identifier` no console,
  mudando de palavra a cada deploy**: é o bug documentado em detalhe no `CHANGELOG.md`
  (2026-08-26) — o `HtmlService` do Apps Script corrompe HTML grande servido como resposta de Web
  App. Não tem correção conhecida além de não usar `HtmlService` pra isso — ver o changelog antes
  de tentar de novo.
